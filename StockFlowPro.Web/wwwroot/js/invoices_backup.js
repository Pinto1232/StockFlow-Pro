        ...
        if (response.ok) {
            ...
            if (!authStatus.isAuthenticated) {
                ...
                if (authStatus.userId) {
                    ...
                    console.log('🔴 User ID is null/undefined - skipping sync check');
                    console.log('🔴 Auth status:', authStatus);
                    showPermissionWarning('Unable to verify user account. Please try logging out and logging back in.');
                    disableInvoiceCreation();
                }
            }
        } else {
...
async function loadInvoices() {
    try {
        showLoadingState();
        const response = await fetch(`/api/invoices?pageNumber=${currentPage}&pageSize=${itemsPerPage}`);
        if (response.ok) {
...
function createInvoiceRowHTML(invoice) {
    // Use new invoice properties if available, fallback to legacy properties
    const invoiceNumber = invoice.invoiceNumber || `INV-${invoice.id.substring(0, 8).toUpperCase()}`;
    const customerName = escapeHtml(invoice.customerName || 'Unknown Customer');
    const issueDate = invoice.issueDate ? new Date(invoice.issueDate).toLocaleDateString() : 
                     (invoice.createdDate ? new Date(invoice.createdDate).toLocaleDateString() : 'N/A');
    const dueDate = invoice.dueDate ? new Date(invoice.dueDate).toLocaleDateString() : 'N/A';
    const totalItemCount = invoice.items ? invoice.items.length : (invoice.totalItemCount || 0);
    
    // Use new totalAmount property if available, fallback to legacy total
    const totalAmount = invoice.totalAmount || invoice.total || 0;
    const total = window.FormattingUtils ? 
        window.FormattingUtils.formatCurrency(totalAmount) : 
        `R${totalAmount.toFixed(2)}`;
    
    // Use new status property if available, fallback to isActive
    let statusBadge;
    if (invoice.status) {
        const statusClass = getStatusBadgeClass(invoice.status);
        statusBadge = `<span class="badge ${statusClass}">${escapeHtml(invoice.status)}</span>`;
    } else {
        statusBadge = invoice.isActive ? 
