...
const Products: React.FC = () => {
  ...
  const [consoleEntries, setConsoleEntries] = useState<Array<{
    ...
    {
      timestamp: '[System]',
      message: 'Product Management Console initialized - Ready to log database operations',
      type: 'system'
    }
  ]);
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      addConsoleEntry('Database connection established - Products loaded successfully', 'database');
    }, 1000);
  }, []);

  const addConsoleEntry = (message: string, type: 'system' | 'database' | 'api' | 'error') => {
    const timestamp = new Date().toLocaleTimeString();
    setConsoleEntries(prev => [...prev, { timestamp: `[${timestamp}]`, message, type }]);
  };
      ...
      <nav className="bg-white/90 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-50 w-full px-4 sm:px-6 lg:px-8 py-4">
        <ol className="flex items-center gap-2 text-sm">
          <li className="flex items-center gap-2">
            <Link to="/dashboard" className="flex items-center gap-2 text-blue-600 hover:bg-blue-50 px-3 py-2 rounded-lg transition-colors font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="relative bg-white rounded-2xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Total Products</div>
                <div className="text-2xl font-bold text-gray-900">{totalProducts}</div>
                <small className="text-gray-400 text-xs">Enhanced formatting</small>
              </div>
            </div>
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Total Value</div>
                <div className="text-2xl font-bold text-gray-900">${totalValue.toLocaleString()}</div>
                <small className="text-gray-400 text-xs">Currency & short format</small>
              </div>
            </div>
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Low Stock</div>
                <div className="text-2xl font-bold text-gray-900">{lowStockCount}</div>
                <small className="text-gray-400 text-xs">Count & percentage</small>
              </div>
            </div>
          ...
          <div className="relative bg-white rounded-2xl p-6 shadow-lg border border-gray-200 hover:shadow-xl transition-all duration-300 hover:-translate-y-1">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-2">Out of Stock</div>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setLowStockOnlyFilter(e.target.checked)}
                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                />
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-gray-200 mb-6 overflow-hidden">
            ...
            <div className="bg-gray-900 text-gray-100 p-4 max-h-80 overflow-y-auto font-mono text-sm">
              {consoleEntries.map((entry, index) => (
                <div key={index} className="flex gap-3 py-1 border-b border-gray-700/30 last:border-b-0">
                  <span className={`text-xs font-medium min-w-fit ${
                    entry.type === 'system' ? 'text-blue-400' :
                    entry.type === 'database' ? 'text-green-400' :
                    entry.type === 'api' ? 'text-yellow-400' : 'text-red-400'
                  }`}>
                    {entry.timestamp}
                  </span>
                  <span className={`${
                    entry.type === 'system' ? 'text-blue-200' :
                    entry.type === 'database' ? 'text-green-200' :
                    entry.type === 'api' ? 'text-yellow-200' : 'text-red-200'
                  }`}>
                    {entry.message}
                  </span>
                </div>
            ...
            <div className="rounded-2xl overflow-hidden">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <h5 className="text-lg font-semibold text-gray-600 mb-2">No products found</h5>
                ...
                <table className="w-full m-0 border-separate border-spacing-0">
                  <thead>
                    <tr>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Name</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Cost per Item</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Stock</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Total Value</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Status</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Created</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {paginatedProducts.map((product, index) => (
                      <tr key={product.id} className="hover:bg-[#f8fafc] transition-colors duration-200">
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''} font-medium text-gray-900`}>{product.name}</td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>${product.costPerItem.toFixed(2)}</td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''}`}>
                          <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${getStockBadgeClasses(product.stock)}`}>
                            {product.stock}
                          </span>
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>${product.totalValue.toFixed(2)}</td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''}`}>
                          <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${getStatusBadgeClasses(product.status)}`}>
                            {product.status}
                          </span>
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>{product.createdAt}</td>
                        <td className={`align-middle text-sm p-4 ${index !== paginatedProducts.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-right`}>
