      ...
      <nav className="bg-white/90 backdrop-blur-sm border-b border-gray-200 sticky top-0 z-50 w-full px-4 sm:px-6 lg:px-8 py-4">
        <ol className="flex items-center gap-2 text-sm">
          <li className="flex items-center gap-2">
            <a href="/dashboard" className="flex items-center gap-2 text-blue-600 hover:bg-blue-50 px-3 py-2 rounded-lg transition-colors font-medium">
        ...
        <div className="bg-white rounded-2xl p-6 shadow-lg border border-gray-200 mb-6">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                <label htmlFor="userFilter" className="block text-sm font-medium text-gray-700 mb-2">Created By</label>
                <select 
                  className="w-full px-3 py-2 border-2 border-gray-200 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all duration-200" 
                  id="userFilter"
                  value={userFilter}
            ...
            <div className="rounded-2xl overflow-hidden">
              {filteredInvoices.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12">
                  ...
                  <h5 className="text-lg font-semibold text-gray-600 mb-2">No invoices found</h5>
                ...
                <table className="w-full m-0 border-separate border-spacing-0">
                  <thead>
                    <tr>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Invoice #</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Date Created</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Created By</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Items</th>
                      <th className="bg-gradient-to-r from-[#f8fafc] to-[#f1f5f9] border-b border-[#e2e8f0] font-semibold text-xs uppercase tracking-[0.5px] text-[#475569] p-4 sticky top-0 z-10 text-left">Total</th>
                  ...
                  <tbody>
                    {currentInvoices.map((invoice, index) => (
                      <tr key={invoice.id} className="hover:bg-[#f8fafc] transition-colors duration-200">
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} font-medium text-gray-900`}>
                          <span className="invoice-number">
                            {getInvoiceNumber(invoice.id)}
                          </span>
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>
                          {formatDate(invoice.createdDate)}
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>
                          {invoice.createdByUserName || 'Unknown User'}
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>
                          {invoice.totalItemCount || 0}
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-gray-700`}>
                        ...
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''}`}>
                          <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium ${invoice.isActive ? 'bg-gradient-to-r from-green-500 to-green-600 text-white shadow-[0_2px_8px_rgba(16,185,129,0.3)]' : 'bg-[#e2e8f0] text-[#64748b]'}`}>
                            {invoice.isActive ? 'Active' : 'Inactive'}
                          </span>
                        </td>
                        <td className={`align-middle text-sm p-4 ${index !== currentInvoices.length - 1 ? 'border-b border-[#f1f5f9]' : ''} text-right`}>
                          <div className="flex justify-end gap-2">
                            ...
                            <div className="relative group">
                              ...
                              <div className="absolute right-0 top-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-2 min-w-[120px] z-[60] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
                                ...
                                <button 
                                  className="flex items-center gap-2 w-full px-3 py-2 text-sm text-gray-700 hover:bg-gray-100 rounded transition-colors"
                                  onClick={() => handleDownload(invoice.id, 'json')}
                                >
